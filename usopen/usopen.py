...
def main():
    ...
    print("\n***** BEGIN BOOTSTRAPPER CHECK*****")
    config = input("\nWould you like a apply a new configuration? y/n\n>")
    if config.lower() in ['y', 'n']:
        conf_loc = input("\nEnter the name of new config file.\n>")
        conf_ip = input("\nHostname of the device to be configured? \n>")
        if bootstrapper.bootstrapper(f"{entry[switch]}",
                                    switchip, 
                                    "admin", 
                                    "alta3",
                                    conf_loc):
